...
object rvcModule{
	/**
	 * RVC instruction identifier
	 *
	 * inst Bits-type 32-bits unknown instruction
	 * @return Bool-type value, low means RVC instruction, high means other 32-bits instruction
	 */
	case class rvcCheck() extends TouchArea{
		def apply(inst: Bits):Bool = inst(1 downto 0) === B"2'b11"
	}
	/**
	 * RVC instruction set expander, which changes RVC instruction to standard RV32 instruction
	 *
	 * rvc_inst Bits-type 16-bits RVC instruction
	 * @return Bit-type 32-bits RV32 instruction type
	 */
	case class rvcExpander() extends TouchArea {
		private def err_code = B"32'hFFFFFFFF"

		def apply(rvc_inst: Bits): Bits = {
